...
public class ApiBootstrapClient {
    ...
    private void createReservation(String token, long spaceId, ReservationCriteria criteria) {
        try {
            LocalDateTime startDateTime = criteria.date().atTime(Integer.parseInt(criteria.startTime().substring(0, 2)), 0);
            LocalDateTime endDateTime = criteria.date().atTime(Integer.parseInt(criteria.endTime().substring(0, 2)), 0);
